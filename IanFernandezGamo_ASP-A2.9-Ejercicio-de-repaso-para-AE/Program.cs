...
public class EmpleadoPorHora : Empleado
{
    ...
    public int HorasTrabajadasMes
    {
        ...
        set { horasTrabajadasMes = (value < 0) ? 0 : value; }
    }
    ...
    public override string ToString()
    {
        return base.ToString() + $", tarifa por hora: {TarifaHora}€, horas trabajadas al mes: {HorasTrabajadasMes}";
    }
}
            ...
            static void contratarEmpleado(List<Empleado> empleados)
{
    ...
    // Leer salario base con TryParse
    double salarioBase;
    Console.Write("Salario Base: ");
    string salarioInput = Console.ReadLine();
    if (!double.TryParse(salarioInput, System.Globalization.NumberStyles.Number, System.Globalization.CultureInfo.CurrentCulture, out salarioBase))
    {
        Console.WriteLine("Entrada de salario no válida. Se usará 0.0");
        salarioBase = 0.0;
    }
    Console.WriteLine($"DEBUG: salarioBase leído = {salarioBase}");

    if (tipoEmpleado == "1")
    {
        double bonoAnual;
        Console.Write("Bono Anual: ");
        string bonoInput = Console.ReadLine();
        if (!double.TryParse(bonoInput, System.Globalization.NumberStyles.Number, System.Globalization.CultureInfo.CurrentCulture, out bonoAnual))
        {
            Console.WriteLine("Entrada de bono no válida. Se usará 0.0");
            bonoAnual = 0.0;
        }
    ...
    else if (tipoEmpleado == "2")
    {
        ...
        if (!double.TryParse(tarifaInput, System.Globalization.NumberStyles.Number, System.Globalization.CultureInfo.CurrentCulture, out tarifaHora))
        {
            Console.WriteLine("Entrada de tarifa no válida. Se usará 0.0");
            tarifaHora = 0.0;
        }
        ...
        if (!int.TryParse(horasInput, out horasTrabajadasMes))
        {
            Console.WriteLine("Entrada de horas no válida. Se usará 0");
